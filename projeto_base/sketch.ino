...
// Alvos da cultura (Cana)
const float phMin = 5.5, phMax = 6.5;
const float umidadeLiga = 55.0;     // liga se < 55%
const float umidadeDesliga = 70.0;  // desliga se > 70%
...
void setup() {
  ...
  Serial.println("FarmTech - Irrigacao Inteligente (toggle em pushbuttons)");
}
...
void loop() {
  ...
  float hum = isnan(th.humidity) ? 50.0 : th.humidity;

  bool nutrientesOK = latched[0] && latched[1] && latched[2]; // N, P, K
